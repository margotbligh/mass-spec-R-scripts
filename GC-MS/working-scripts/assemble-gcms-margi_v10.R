...
pdp <- PeakDensityParam(sampleGroups = data_peaks$SampleGroup,
...
pdp <- PeakDensityParam(sampleGroups = peaks_RTaligned$SampleGroup,
...
sampclass(xset) <- pData(peaks_final)$SampleGroup
an <- xsAnnotate(xset)
...
pspectra.r.alg.met$ps_nr %>% unique %>% length() #203
...
blank.wilcox.df.sig$ps_nr %>% unique() %>% length() #n = 120
...
wilcox.df.sig$ps_nr %>% unique() %>% length() #n = 13
...
ps_nr.no <- c(ps_nr.no, 10, 73, 118, 137, 159, 219, 222)
...
pspectra.r.alg.sig$compound[pspectra.r.alg.sig$ps_nr == 19] <- "alcohol-like"
pspectra.r.alg.sig$compound[pspectra.r.alg.sig$ps_nr == 20] <- "small organic acid-like"
...
wilcox.df.sig$compound[wilcox.df.sig$ps_nr == 19] <- "alcohol-like"
wilcox.df.sig$compound[wilcox.df.sig$ps_nr == 20] <- "small organic acid-like"
wilcox.df.sig$compound[wilcox.df.sig$ps_nr == 44] <- "small aromatic-like"

compound_names <- list("13"="mannitol", "19"="alcohol-like",
                       "20"="small organic acid-like", "44"="small aromatic-like")
...
#25: Relative quantification after ribitol normalisation by weighted regression, SEAGRASS + stds + blanks only----

pd.sg<- pd[pd$Plant == "SEAGRASS" | pd$Condition == "blank" | pd$Condition == "metabolitestd" ,]
pd.sg <- pd.sg %>% filter(Enrichment13C == "NO") %>% 
  filter(WaterBody != "PW")

spclist.by.sample.sg <- spclist.by.sample[which(pd$SampleName %in% pd.sg$SampleName)]
names(spclist.by.sample.sg) <- which(pd$SampleName %in% pd.sg$SampleName)

pspectra.r.sg <- data.frame(sample_name = rep(pd.sg$SampleName, length(pspectra)),
                             sample_nr = rep(names(spclist.by.sample.sg), length(pspectra)),
                             ps_nr = rep(1:length(pspectra), each = nrow(pd.sg)),
                             rel_intensity = NA, ref_sample = NA)

std.n <- pd.sg$sample_nr[pd.sg$Condition == "metabolitestd"]
blank.n <- pd.sg$sample_nr[pd.sg$Condition == "blank"]
seawater.n  <- pd.sg$sample_nr[pd.sg$SamplingTime == "I"]
inc.n <- pd.sg$sample_nr[pd.sg$SamplingTime == "E"]

#use as a reference spectra the spectra that has all ions not NA
#priority: standards, blanks, samples
for (i in 1:length(pspectra)){
  x <- lapply(spclist.by.sample.sg, "[[", i)
  index <- vector()
  for (n in 1:length(std.n)){
    a <- x[[as.character(std.n[n])]]
    if(any(is.na(a))){
      next
    } else if(!any(is.na(a))){index <- c(index, std.n[n])
    }
  ...
  if (length(index) == 1){
    refspec <- spclist.by.sample.sg[[as.character(index)]][[i]]
    refspec <- removeDoubleMasses(refspec)
  } else if (length(index)==2){
    spec1 <- spclist.by.sample.sg[[as.character(index[1])]][[i]]
    spec1 <- removeDoubleMasses(spec1)
    spec2 <- spclist.by.sample.sg[[as.character(index[2])]][[i]]
    spec2 <- removeDoubleMasses(spec2)
    rel <- relInt(spec1, spec2)
    if (rel < 1) {
      index <- index[2]
    ...
    spec2 <- spclist.by.sample.sg[[as.character(index[2])]][[i]]
    spec2 <- removeDoubleMasses(spec2)
    spec3 <- spclist.by.sample.sg[[as.character(index[3])]][[i]]
    spec3 <- removeDoubleMasses(spec3)
    rel1v2 <- relInt(spec1, spec2)
    rel1v3 <- relInt(spec1, spec3)
    rel2v3 <- relInt(spec2, spec3)
  ...
  pspectra.r.sg$ref_sample[pspectra.r.sg$ps_nr == i] <- index
  for (j in 1:nrow(pd.sg)){
    s = pd.sg$SampleName[j]
    spec <- na.omit(spclist.by.sample.sg[[j]][[i]])
    spec <- removeDoubleMasses(spec)
    pspectra.r.sg$rel_intensity[pspectra.r.sg$ps_nr == i & 
                                  pspectra.r.sg$sample_name == s] <-  relInt(spec, refspec)
  }
}
names(pspectra.r.sg)[names(pspectra.r.sg) == "sample_name"] <- "SampleName"
pspectra.r.sg$sample_nr <- as.numeric(pspectra.r.sg$sample_nr)
pspectra.r.sg.met <- left_join(pspectra.r.sg, pd.sg, by = c("SampleName", "sample_nr"))



#26: Plot intensities after relative quantification----
pspectra.r.sg.met$SampleGroup.n <- factor(pspectra.r.sg.met$SampleGroup.n,
                                           levels= c("blank", "standard",
                                                     "seagrass\nseawater",
                                                     "seagrass\ndark",
                                                     "seagrass\nlight"))
...
pal.sg <-c("white", "grey", 
           "#DEAF3A",scico(2, palette = "cork",begin=0.75,end=0.6, alpha=0.7))
names(pal.sg) <- levels(pspectra.r.sg.met$SampleGroup.n)

for(i in 1:length(pspectra)){
  x <- pspectra.r.sg.met %>% filter(ps_nr == i)
  if(any(is.na(x$ref_sample))){next}
  p <- ggplot(x, aes(x = SampleGroup.n, y = rel_intensity)) +
    geom_boxplot(mapping = aes(fill=SampleGroup.n), 
                 position = position_dodge(width = 0.8), alpha = 0.7) + 
    geom_point(mapping = aes(fill=SampleGroup.n), pch=21, size=2, 
               position = position_dodge(width = 0.8), alpha = 0.7) +
    scale_fill_manual(values = pal.sg, name = "")+
    labs(x = "", title = paste("Pseudospectra", i, "- intensities relative to",
                               pd.sg$SampleGroup[unique(x$ref_sample)],
                               "( sample number", unique(x$ref_sample), ")")) +
    theme_bw() + 
    theme(text=element_text(size=font_size, family = font_family, colour = "#262626"),
          axis.text = element_text(size=font_size, family = font_family, colour = "#262626"),
  ...
  png(filename = paste0("plots/relative-quantification-seagrass/", i, ".png"),
      height = 12, width = 9, res = 300, units = "in")
  print(p)
  dev.off()
}

#27: Blank subtraction----
pspectra.r.sg.met.all <- pspectra.r.sg.met

#remove any spectra with no ref sample
pspectra.r.sg.met <- pspectra.r.sg.met.all[complete.cases(pspectra.r.sg.met.all),]
pspectra.r.sg.met$ps_nr %>% unique %>% length() #204
a <- pspectra.r.sg.met$ps_nr %>% unique
#do wilcox tests for all groups vs blanks (seagrass only)
#get column names for table
i = 1
ps_nr = a[i]
df1 <- pspectra.r.sg.met %>% filter(ps_nr == !!ps_nr)
df1 <- df1 %>% 
  rstatix::wilcox_test(rel_intensity ~ SampleGroup, ref.group = "blank",
                       alternative = "less") %>% 
  rstatix::adjust_pvalue(p.col = "p", method = "BH") %>%
...
blank.wilcox.df.sig <- blank.wilcox.df %>% filter(p <= 0.05)
blank.wilcox.df.sig$ps_nr %>% unique() %>% length() #n = 139

#28: Significance testing (wilcox)----
#do wilcox tests for dark and light incubations vs seawater (seagrass only)
pspectra.r.sg.met.samples <- pspectra.r.sg.met %>% 
  filter(SampleGroup != "blank" & SampleGroup != "standard") %>% 
  filter(ps_nr %in% blank.wilcox.df.sig$ps_nr)

ps_nr.tests <- pspectra.r.sg.met.samples$ps_nr %>% unique()
#get column names for table
i = 1
ps_nr = ps_nr.tests[i]
df1 <- pspectra.r.sg.met.samples %>% filter(ps_nr == !!ps_nr)
df1 <- df1 %>% 
  rstatix::wilcox_test(rel_intensity ~ SampleGroup, ref.group = "seagrass seawater") %>% 
  rstatix::adjust_pvalue(p.col = "p", method = "BH") %>%
  rstatix::add_significance(p.col = "p.adj") %>% 
  rstatix::add_xy_position()
cols <- cbind(ps_nr = NA, cols)
wilcox.df.sg <- cols[!any(is.na(cols)),]
rm(cols)
...
for(i in 1:length(ps_nr.tests)){
  ...
  if(ps_nr == 23){next} #ribitol
    ...
    df2 <- df1 %>% 
      ...
      rstatix::add_xy_position()
    df2 <- cbind(ps_nr = ps_nr, df2)
    wilcox.df.sg <- rbind(wilcox.df.sg, df2)
  }
}

wilcox.df.sg.sig <- wilcox.df.sg %>% filter(p.adj <= 0.05)
wilcox.df.sg.sig$ps_nr %>% unique() %>% length() #n = 3

#29: Plot intensities of ps with significant differences-----
#dataframe. only keep water column
pspectra.r.sg.sig <- pspectra.r.sg.met[pspectra.r.sg.met$ps_nr %in% wilcox.df.sg.sig$ps_nr,] 
pspectra.r.sg.sig <- pspectra.r.sg.sig %>% filter(SampleGroup != "standard")
pspectra.r.sg.sig$SampleGroup <- factor(pspectra.r.sg.sig$SampleGroup,
                                         levels = c("blank",
                                                    "seagrass seawater",
                                                    "seagrass dark",
                                                    "seagrass light"))
#to remove if lower than seawater or blank
ps_nr.no <- c(13, 216)
pspectra.r.sg.sig <- pspectra.r.sg.sig[!pspectra.r.sg.sig$ps_nr %in% ps_nr.no,]
wilcox.df.sg.sig <- wilcox.df.sg.sig[!wilcox.df.sg.sig$ps_nr %in% ps_nr.no,]
...
#significance dataframe
wilcox.df.sg.sig$xmin <- 2
wilcox.df.sg.sig$xmax[wilcox.df.sg.sig$group2 == "seagrass dark"] <- 3
wilcox.df.sg.sig$xmax[wilcox.df.sg.sig$group2 == "seagrass light"] <- 4


#plot
ggplot(data = pspectra.r.sg.sig, aes(x=SampleGroup, y=rel_intensity)) +
  geom_boxplot(mapping = aes(fill=SampleGroup),
               position = position_dodge(width = 0.8), alpha = 0.7) +
  geom_point(mapping = aes(fill=SampleGroup), pch=21, size=2, 
             position = position_dodge(width = 0.8), alpha = 0.7) +
...
#30: Get putative annotations-----
#annotations based on standard compound table
seagrass.mlist1 <- list()
a <- pspectra.r.sg.sig$ps_nr %>% unique()
for (i in 1:length(a)){
  ps_nr = a[i]
  ps <- bind_rows(lapply(spclist.by.sample, "[[", ps_nr), .id = "sample_nr")
  ps$mzmin <- ps$mz - 0.1
  ...
  df1 <- foverlaps(ps,x)
  df1 <- na.omit(df1)
  seagrass.mlist1[[i]] <- df1 %>% dplyr::group_by(compound, ion) %>% 
    dplyr::summarise(n=n())
}
names(seagrass.mlist1) <- as.character(a)
seagrass.mlist1 <- bind_rows(seagrass.mlist1, .id = "ps_nr")
#annotations by comparison to mona db
seagrass.mona1 <- data.frame(ps_nr = NA, rt = NA,
                             annot1 = NA, dotprod1 = NA, 
                             annot2 = NA,dotprod2 = NA,
                             annot3 = NA,dotprod3 = NA, 
                             annot4 = NA, dotprod4 = NA,
...
for (i in 1:length(a)){
  ...
  tmp.df <- data.frame(ps_nr = ps_nr, rt = rt.spclist[ps_nr],
                       ...
                       annot4 = mona.names[as.numeric(names(l)[4])], dotprod4 = l[4],
                       annot5 = mona.names[as.numeric(names(l)[5])], dotprod5 = l[5])
  seagrass.mona1 <- rbind(seagrass.mona1, tmp.df)
  
}
seagrass.mona1 <- na.omit(seagrass.mona1)

seagrass.mona2 <- seagrass.mona1 %>% 
  pivot_longer(cols = contains("annot"), names_to = "annot_n", values_to = "annot") %>% 
  pivot_longer(cols = contains("dotprod"), names_to = "dotprod_n", values_to = "dotprod")
seagrass.mona2$annot_n <- sub("annot", "", seagrass.mona2$annot_n) %>% as.numeric()
seagrass.mona2$dotprod_n <- sub("dotprod", "", seagrass.mona2$dotprod_n) %>% as.numeric()
seagrass.mona2 <- seagrass.mona2[seagrass.mona2$annot_n == seagrass.mona2$dotprod_n,]

#31: Plot seagrass-------
compound_names.sg <- list("123"="benzoic acid-like")
compound_names.sg <- unlist(compound_names.sg)

time_cat_5colors<-c("white", scico(1, palette = "cork",begin=0.6,end=0.7, alpha=0.7),
                    scico(2, palette = "vikO",begin=0.25,end=0.75, alpha=0.7))

ggplot(data = pspectra.r.sg.sig)+
  ...
  facet_wrap(~ps_nr, scales = "free_y", nrow = 1,
             labeller = as_labeller(compound_names.sg)) +
  add_pvalue(wilcox.df.sg.sig, xmin = "xmin", xmax = "xmax", 
             y.position = wilcox.df.sg.sig$y.position,
             label = "p.adj.signif",
...
#algae
#no data for ps 123, was no reference for it to calculate relative to
#so make dummy data 
pspectra.r.alg.sig2 <- pspectra.r.alg.sig
n <- nrow(pspectra.r.alg.sig)+1
pspectra.r.alg.sig2[n,] <- NA
pspectra.r.alg.sig2$ps_nr[n] <- 123
...
png(file = "figs/gc-ms_algae_20220208.png", width = 10, height = 3, units = "in",
    res = 300)
p1
dev.off()
...
png(file = "figs/gc-ms_seagrass_20220208.png", width = 10, height = 3, units = "in",
    res = 300)
p2
dev.off()
